      ...
      <div className='bg-[#EBEDF1] h-[92vh] flex justify-center items-center'>
        <div className='p-10 bg-white w-[94%] h-[80vh] shadow-md overflow-hidden'>
          ...
          <div className='flex flex-col md:flex-row justify-between items-center mb-6'>
            <h1 className='text-[#8A2A2B] text-xl font-bold text-center md:text-left'>อนุมัติการจองห้องประชุม</h1>
            ...
            <table className='w-full table-auto'>
              <thead>
                <tr className="border-b">
                  ...
                  <th className="w-[25%] text-center p-4 hidden sm:table-cell">เรื่อง</th>
              ...
              <tbody>
                {TABLE_ROWS.map(({ id, room, date, time, topic, status_, manage }, index) => (
                  <tr key={index} className='border-b border-gray-400'>
                    ...
                    <td className="text-center px-4 py-5 hidden sm:table-cell">{topic}</td>
                    ...
                    <td className='text-center px-4 py-5'>
                      <Link to="/admin/approvebooking" >
                        <button className='w-20 h-9 bg-[#3B65FB] text-white rounded-md text-sm cursor-pointer hover:bg-[#2650E6]'>{manage}</button>
                      </Link>
                    </td>
                  </tr>
                ))}
